...
#define VDB_HEADER_SIZE ((sizeof(uint64_t) * 3) + 4)
...
int VDB_get(VDB *db, const void *key, void *vbuf) {
    ...
    uint64_t hash = VDB_hash(key, db->key_size) % (uint64_t)db->hash_table_size;
    uint64_t offset;
...
int VDB_put(VDB *db, const void *key, const void *value) {
    ...
    lasthtoffset = htoffset = VDB_HEADER_SIZE;
    cur_hash_table = db->hash_tables;
...
int main(int argc, char **argv) {
    ...
    printf("Opening new empty database sample db\n");

    if (VDB_open(&db, "sample.db", VDB_OPEN_MODE_RWREPLACE, 1024, 8, sizeof(v))) {
        printf("VDB_open failed \n");
        return 1;
    }
    ...
    for (i = 0; i < 10000; ++i) {
        for (j = 0; j < 8; ++j)
            v[j] = i;
        if (VDB_put(&db, &i, v)) {
            printf("VDB_put failed (%"PRIu64")\n", i);
            return 1;
        }

        memset(v, 0, sizeof(v));
        if (( q = VDB_get(&db, &i, v))) {
            printf("VDB_get (1) failed (%"PRIu64") (%d)\n", i, q);
            return 1;
        }

        for (j = 0; j < 8; ++j) {
            if(v[j] != i) {
                printf("VDB_get (1) failed, bad data (%"PRIu64")\n", i);
                return 1;
            }
        }
    }
    ...
    for( i = 0; i < 10000; ++i) {
        if ((q = VDB_get(&db, &i, v))) {
            printf("VDB_get (2) failed (%"PRIu64") (%d)\n", i, q);
            return 1;
        }

        for (j = 0; j < 8; ++j) {
            if (v[j] != i) {
                printf("VDB_get (3) failed, bad data (%"PRIu64")\n", i);
                return 1;
            }
        }
    }
    ...
    VDB_Iterator_init(&db, &dbi);
    i = 0xdeadbeef;
    memset(got_all_values, 0, sizeof(got_all_values));

    while(VDB_Iterator_next(&dbi, &i, &v) > 0) {
        if (i < 10000)
            got_all_values[i] = 1;
        else {
            printf("VDB_Iterator_next failed, bad data (%"PRIu64")\n", i);
            return 1;
        }
    }
    ...
    for (i = 0; i < 10000; ++i) {
        if (!got_all_values[i]) {
            printf("VDB_Iterator failed, missing value index (%"PRIu64")\n", i);
            return 1;
        }
    }
